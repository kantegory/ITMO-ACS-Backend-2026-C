...
func (c *bookingClient) getByID(bookingID int64, accessToken string) (*bookingResponse, int, string) {
	...
	if resp.StatusCode == http.StatusForbidden {
		return nil, http.StatusForbidden, "Недостаточно прав для отзыва по этому бронированию"
	}
...
func (c *bookingClient) getByIDInternal(bookingID int64, serviceToken string) (*bookingResponse, int, string) {
	url := strings.TrimRight(c.baseURL, "/") + "/internal/bookings/" + strconv.FormatInt(bookingID, 10)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	if err != nil {
		return nil, http.StatusServiceUnavailable, "Не удалось проверить бронирование"
	}
	...
	if resp.StatusCode == http.StatusUnauthorized {
		return nil, http.StatusUnauthorized, "Неавторизован"
	}
	if resp.StatusCode != http.StatusOK {
		return nil, http.StatusServiceUnavailable, "Не удалось проверить бронирование"
	}
