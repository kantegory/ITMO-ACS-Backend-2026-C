...
func (c *propertyClient) getByID(propertyID int64) (*propertyResponse, int, string) {
	...
	if err != nil {
		return nil, http.StatusServiceUnavailable, "Не удалось проверить объект"
	}

	resp, err := c.httpClient.Do(req)
	if err != nil {
		return nil, http.StatusServiceUnavailable, "Сервис недвижимости недоступен"
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&body); err != nil {
		return nil, http.StatusServiceUnavailable, "Ошибка ответа сервиса недвижимости"
	}
	if body.ID <= 0 {
		return nil, http.StatusNotFound, "Объект не найден"
	}
