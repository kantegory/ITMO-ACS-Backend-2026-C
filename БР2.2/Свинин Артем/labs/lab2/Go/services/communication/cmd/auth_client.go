...
func (c *authClient) authenticate(accessToken string) (principal, int, string) {
	...
	if !body.Valid || body.UserID <= 0 || !body.IsActive {
		return principal{}, http.StatusUnauthorized, "Невалидный токен"
	}

	return principal{UserID: body.UserID, Role: strings.ToUpper(strings.TrimSpace(body.Role)), Email: strings.TrimSpace(body.Email)}, http.StatusOK, ""
}
